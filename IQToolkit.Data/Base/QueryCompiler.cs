...
namespace IQToolkit
{
	...
	public static class QueryCompiler
	{
		...
		public static Func<T1, T2, T3, T4, TResult> Compile<T1, T2, T3, T4, TResult>(
			Expression<Func<T1, T2, T3, T4, TResult>> query)
		{
				...
				if (this.fnQuery != null && this.invoker == null && !checkedForInvoker)
				{
					...
					if (fnType.FullName.StartsWith("System.Func`"))
					{
						...
						MethodInfo method = this.GetType().GetMethod(
							"FastInvoke" + typeArgs.Length, BindingFlags.Public | BindingFlags.Instance);
						if (method != null)
						{
							this.invoker =
								(Func<object[], object>)
								Delegate.CreateDelegate(typeof(Func<object[], object>), this, method.MakeGenericMethod(typeArgs));
						}
					}
				}
				return this.invoker;
