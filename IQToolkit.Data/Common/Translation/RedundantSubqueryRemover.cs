...
namespace IQToolkit.Data.Common
{
	...
	public class RedundantSubqueryRemover : DbExpressionVisitor
	{
		...
		private class RedundantSubqueryGatherer : DbExpressionVisitor
		{
			...
			{
				return (IsSimpleProjection(select) || IsNameMapProjection(select)) && !select.IsDistinct && !select.IsReverse
				       && select.Take == null && select.Skip == null && select.Where == null
		...
		private class SubqueryMerger : DbExpressionVisitor
		{
			...
			protected override Expression VisitSelect(SelectExpression select)
			{
				...
				while (CanMergeWithFrom(select, wasTopLevel))
				{
					...
					if (where != select.Where || orderBy != select.OrderBy || groupBy != select.GroupBy
					    || isDistinct != select.IsDistinct || skip != select.Skip || take != select.Take)
					{
						select = new SelectExpression(
							select.Alias, select.Columns, select.From, where, orderBy, groupBy, isDistinct, skip, take, select.IsReverse);
					}
				}
			...
			private static bool CanMergeWithFrom(SelectExpression select, bool isTopLevel)
			{
				...
				if (fromSelect.Take != null
				    &&
				    (select.Take != null || select.Skip != null || select.IsDistinct || selHasAggregates || selHasGroupBy
				     || selHasJoin))
				{
					return false;
				}
				// cannot move forward a skip if outer has skip or distinct
				if (fromSelect.Skip != null
				    && (select.Skip != null || select.IsDistinct || selHasAggregates || selHasGroupBy || selHasJoin))
				{
					return false;
				}
				// cannot move forward a distinct if outer has take, skip, groupby or a different projection
				if (fromSelect.IsDistinct
				    &&
				    (select.Take != null || select.Skip != null || !selHasNameMapProjection || selHasGroupBy || selHasAggregates
				     || (selHasOrderBy && !isTopLevel) || selHasJoin))
				{
					return false;
				}
				if (frmHasAggregates
