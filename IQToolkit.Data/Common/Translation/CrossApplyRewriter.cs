		...
		protected override Expression VisitJoin(JoinExpression join)
		{
			...
			if (join.Join == JoinType.CrossApply || join.Join == JoinType.OuterApply)
			{
				...
				else
				{
					...
					if (select != null && select.Take == null && select.Skip == null && !AggregateChecker.HasAggregates(select)
					    && (select.GroupBy == null || select.GroupBy.Count == 0))
					{
						...
						if (referencedAliases.Count == 0)
						{
							...
							var pc = ColumnProjector.ProjectColumns(
								where, select.Columns, select.Alias, DeclaredAliasGatherer.Gather(select.From));
							select = select.SetColumns(pc.Columns);
							where = pc.Projector;
							JoinType jt = (where == null)
								              ? JoinType.CrossJoin
								              : (join.Join == JoinType.CrossApply ? JoinType.InnerJoin : JoinType.LeftOuter);
