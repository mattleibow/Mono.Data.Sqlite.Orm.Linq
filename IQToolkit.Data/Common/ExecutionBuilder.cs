...
namespace IQToolkit.Data.Common
{
	...
	public class ExecutionBuilder : DbExpressionVisitor
	{
		...
		private static Expression MakeSequence(IList<Expression> expressions)
		{
			...
			return
				Expression.Convert(
					Expression.Call(typeof(ExecutionBuilder), "Sequence", null, Expression.NewArrayInit(typeof(object), expressions)),
					last.Type);
		}
		...
		protected override Expression VisitClientJoin(ClientJoinExpression join)
		{
			...
			if (join.Projection.Projector.NodeType == (ExpressionType)DbExpressionType.OuterJoined)
			{
				LambdaExpression pred =
					Expression.Lambda(
						Expression.PropertyOrField(kvp, "Value").NotEqual(TypeHelper.GetNullConstant(join.Projection.Projector.Type)), kvp);
			...
			Expression toLookup = Expression.Call(
				typeof(Enumerable),
				"ToLookup",
				new Type[] { kvp.Type, outerKey.Type, join.Projection.Projector.Type },
				execution,
				keySelector,
				elementSelector);
		...
		protected override Expression VisitBatch(BatchExpression batch)
		{
			...
			else
			{
				...
				return Expression.Call(
					this.GetType(),
					"Batch",
					new Type[] { TypeHelper.GetElementType(source.Type), batch.Operation.Body.Type },
					source,
					fn,
					batch.Stream);
			}
		...
		private Expression BuildExecuteBatch(BatchExpression batch)
		{
			...
			{
				plan = Expression.Call(
					this.executor, "ExecuteBatch", null, Expression.Constant(command), paramSets, batch.BatchSize, batch.Stream);
			}
			...
			if (decl.Source != null)
			{
				...
				var projection = new ProjectionExpression(
					...
					Expression.NewArrayInit(
						...
						decl.Variables.Select(
							v => v.Expression.Type.IsValueType ? Expression.Convert(v.Expression, typeof(object)) : v.Expression).ToArray()),
				...
				for (int i = 0, n = decl.Variables.Count; i < n; i++)
				{
					...
					NamedValueExpression nv = new NamedValueExpression(
						v.Name, v.QueryType, Expression.Convert(Expression.ArrayIndex(vars, Expression.Constant(i)), v.Expression.Type));
		...
		private Expression BuildExecuteCommand(CommandExpression command)
		{
			...
			Expression plan = Expression.Call(
				this.executor, "ExecuteCommand", null, Expression.Constant(qc), Expression.NewArrayInit(typeof(object), values));
		...
		private class OuterParameterizer : DbExpressionVisitor
		{
			...
			private Dictionary<ColumnExpression, NamedValueExpression> map =
