...
namespace IQToolkit.Data.Common
{
	...
	{
		private AdvancedMapping mapping;
		...
		public virtual IEnumerable<MappingTable> GetDependencyOrderedTables(MappingEntity entity)
		{
			...
			return
				this.mapping.GetTables(entity).Sort(
					t => this.mapping.IsExtensionTable(t) ? lookup[this.mapping.GetExtensionRelatedAlias(t)] : null);
		}
			...
			foreach (MappingTable table in tables.Where(t => this.mapping.IsExtensionTable(t)))
			{
				...
				for (int i = 0, n = keyColumns.Count; i < n; i++)
				{
					...
					var relatedColumn = new ColumnExpression(
						memberType, colType, relatedTableAlias, this.mapping.GetColumnName(entity, relatedMembers[i]));
		...
		{
			return (from xt in this.mapping.GetTables(entity).Where(t => this.mapping.IsExtensionTable(t))
			        group xt by this.mapping.GetExtensionRelatedAlias(xt)).ToDictionary(
				        g => g.Key, g => g.SelectMany(xt => this.mapping.GetExtensionRelatedMembers(xt)).Distinct().ToList());
		}
				...
				if (this.mapping.IsColumn(entity, m) && fnIncludeColumn(entity, m))
				{
					yield return
						new ColumnAssignment(
							(ColumnExpression)this.GetMemberExpression(table, entity, m), this.GetMemberAccess(instance, m, map));
				}
				else if (this.mapping.IsNestedEntity(entity, m))
				{
					var assignments = this.GetColumnAssignments(
						table, Expression.MakeMemberAccess(instance, m), this.mapping.GetRelatedEntity(entity, m), fnIncludeColumn, map);
			...
			MappingEntity entity, Expression instance, LambdaExpression updateCheck, LambdaExpression selector, Expression @else)
		{
			...
			foreach (var table in this.GetDependencyOrderedTables(entity))
			{
				...
				var assignments = this.GetColumnAssignments(
					...
					instance,
					entity,
					(e, m) => this.mapping.GetAlias(e, m) == this.mapping.GetAlias(table) && this.mapping.IsUpdatable(e, m),
					null);
			...
			if (this.mapping.IsExtensionTable(table))
			{
				...
				for (int i = 0, n = keyColNames.Length; i < n; i++)
				{
					...
					var cex = new ColumnExpression(
						TypeHelper.GetMemberType(relatedMember), this.GetColumnType(entity, relatedMember), root.Alias, keyColNames[n]);
